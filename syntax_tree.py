...
def create_syntax_tree(sentence):
    ...
    return "syntax_tree uses xbar theory without dp hypothesis; you should use syntax_tree_v2 instead."
...
def find_valid_parse(sentence):
    ...
    start_options = ['cp', 'tp', 'np', 'vp', 'pp', 'adjp', 'advp']  # ordered most-to-least likely given no other info
    ...
    if not any([x[1] == 'X' for x in prepared_sentence]):  # i.e., if there are no Xs to worry about
        ...
        for combination in combinations:
            ...
            try:
                output = try_except_lark_parse_helper(prepared_sentence_edited, start_options, sentence)
                break
            except Exception:
...
def grammar(prepared_sentence, start, sentence):
    ...
    lark_input = ''' %import common.WORD
                ...
                tp: (np | cp) T? vp
                vp: v_bar
                v_bar: v_bar pp | v_bar advp | advp v_bar | V np?
                np: D? n_bar
                n_bar: adjp n_bar | n_bar pp | N pp?
                pp: p_bar
                p_bar: p_bar pp | advp p_bar | P np?
                advp: adv_bar 
                ...
                x: x CONJ x '''
    print(lark_input)
    return Lark(lark_input, start=start).parse(sentence)  # if an exception is thrown, it will be here
...
def handle_unaccepted_tag(index, tag, words_and_tags, grammar_pos_map):
    ...
    # if we have gotten here, it means we got a *really* weird tag. Remove it:
    return "remove"
...
if __name__ == "__main__":
    ...
    print(find_valid_parse("The cat in the red hat").pretty())
